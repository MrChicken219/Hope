...
if [ `getconf LONG_BIT` == "32" ]; then
	echo "[ERROR] Foxel is no longer supported on 32-bit systems."
	exit 1
fi
...
if [[ "$BUILD_URL" != "" && "$CHANNEL" == "custom" ]]; then
	...
	VERSION_DATA=$(download_file "https://raw.githubusercontent.com/FoxelTeam/Foxel/master/version.json")
...
if ! [ -s "$NAME.phar" ] || [ "$(head -n 1 $NAME.phar)" == '<!DOCTYPE html>' ]; then
	...
	download_file "https://raw.githubusercontent.com/FoxelTeam/Foxel/master/start.sh" > start.sh
	download_file "https://raw.githubusercontent.com/FoxelTeam/Foxel/master/LICENSE" > LICENSE
	download_file "https://raw.githubusercontent.com/FoxelTeam/Foxel/master/README.md" > README.md
	download_file "https://raw.githubusercontent.com/pmmp/php-build-scripts/master/compile.sh" > compile.sh
fi
